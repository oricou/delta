...
Le but de ce graphe est donc d'exprimer les différentes corrélations entre cette fameuse Échelle de vie, et différentes métriques exprimées dans le GWP : si l'évolution de la métrique est croissante de manière visible, cela veut dire qu'elle est proportionnelle à l'Échelle de vie et donc au bonheur (et l'inverse veut donc dire qu'elle est inversement proportionnelle).
...
La parité de pouvoir d'achat (PPA) est un taux de conversion monétaire qui permet d'exprimer dans une unité commune les pouvoirs d'achat des différentes monnaies. Ce taux exprime la quantité d'unités monétaires nécessaire dans des pays différents pour se procurer le même « panier » de biens et de services.
...
En résulte la moyenne des deux réponses binaires, on peut noter que dans certains pays (comme la Chine, ou le Turkménistan notamment), on utilise uniquement la valeur de la perception de la corruption privée puisque la valeur concernant la corruption publique est manquante.
"""
...
On peut considérer cette valeur comme la perception des gens de leur propre bonheur personnel, indépendamment de leurs conditions de vie et des différents paramètres disponibles.
"""
...
On peut considérer cette valeur comme la perception des gens de leur propre malheur personnel, indépendamment de leurs conditions de vie et des différents paramètres disponibles.
"""

md_help = """
*La navigation dans le graphe est possible en cliquant sur les barres pour ouvrir le détail des valeurs par continents, par pays puis par année. La valeur de comparaison peut également être changée en dessous de ce texte.*
