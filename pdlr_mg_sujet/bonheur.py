...
class Bonheur():
    ...
    def _conv_date(d):
        ...
        return du.parser.parse(f"15-{Bonheur.mois[ma[0].lower()]}-{ma[1]}")  # parfois le mois a une majuscule d'où lower()

    def _make_dataframe_from_pegase(filename):
        df = pd.read_csv(filename, sep=";", encoding="latin1", skiprows=[0,1,3], header=None)
        df = df.set_index(0).T
        df['date'] = df['Période'].apply(Bonheur._conv_date)
    ...
    def __init__(self, application = None):
        bois = Bonheur._make_dataframe_from_pegase("data/pegase_prix_bois_particulier.csv")
        petrole = Bonheur._make_dataframe_from_pegase("data/pegase_prix_petrole_particulier.csv")
        ...
        self.main_layout = html.Div(children=[
            ...
            html.Div([ dcc.Graph(id='nrg-main-graph-1'), ], style={'width':'100%', }),
            html.Div([
                ...
                html.Div([ html.Div('Mois ref.'),
                           dcc.Dropdown(
                               id='nrg-which-month-1',
                               options=[{'label': i, 'value': Bonheur.mois[i]} for i in Bonheur.mois],
                               value=1,
                               disabled=True,
                           ),
        ...
        self.app.callback(
                    dash.dependencies.Output('nrg-main-graph-1', 'figure'),
                    [ dash.dependencies.Input('nrg-price-type', 'value'),
                      dash.dependencies.Input('nrg-which-month-1', 'value'),
                      dash.dependencies.Input('nrg-which-year-1', 'value'),
                      dash.dependencies.Input('nrg-xaxis-type', 'value')])(self.update_graph)
        self.app.callback(
                    [ dash.dependencies.Output('nrg-which-month-1', 'disabled'),
                      dash.dependencies.Output('nrg-which-year-1', 'disabled')],
    ...
    def update_graph(self, price_type, month, year, xaxis_type):
        if price_type == 0 or month == None or year == None:
            ...
            for c in df.columns:
                alias = Bonheur.quoi[c][1]
                try:
                    df[c] = df[c] / (Bonheur.quoi[c][0] * Bonheur.densité[alias] * Bonheur.calor[alias])
                except:  # l'unité est déjà en kg et donc densité n'existe pas
                    df[c] = df[c] / (Bonheur.quoi[c][0] * Bonheur.calor[alias])
        else:
